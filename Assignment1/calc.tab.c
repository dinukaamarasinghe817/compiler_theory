...
{
       0,    60,    60,    61,    61,    62,    71,    80,   102,   122,
     125,   128,   152,   162
};
#endif
...
static const char *const yytname[] =
{
  ...
  "TASSIGN", "TSEMICOLON", "TADD", "TMULT", "TOK_MUL", "TOK_ADD",
  "$accept", "prog", "stmts", "stmt", "expr", YY_NULLPTR
};
...
{
       0,     3,    19,     7,     0,     4,     5,     6,    11,    20,
      21,    11,    11,    11,    12,     8,    13,     9,    10,    11,
      22,    20,    14,    15,    22,    22
};
...
#line 62 "calc.y"
               { // stmt -> int x
    struct symEntry* id = findID((yyvsp[0].variable_type).name);
    if(id == NULL){
        addID((yyvsp[0].variable_type).name, 0);
    }else{
...
#line 71 "calc.y"
    ...
    struct symEntry* id = findID((yyvsp[0].variable_type).name);
    if(id == NULL){
        addID((yyvsp[0].variable_type).name, 1);
...
#line 80 "calc.y"
                   { // stmt -> x = E
    struct symEntry* id = findID((yyvsp[-2].variable_type).name);
    if (id != NULL) { // if an entry exists
        if (id->type == (yyvsp[0].expr_type).type) {
            if(id->type == 0){ // int
                *(int*)id->address = (yyvsp[0].expr_type).value.int_val;
            }else{ // float
                *(float*)id->address = (yyvsp[0].expr_type).value.float_val;
            }
